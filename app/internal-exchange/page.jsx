...
export default function InternalExchangeResortsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Using exactly "International Exchange" as per backend schema
        const citiesRes = await axios.get(
          "/api/cities/region?region=International Exchange&limit=1000",
        );
        ...
        if (hotelsRes.data.success) {
          ...
          if (allHotels) {
            const internalExchangeHotels = allHotels.filter(
              (hotel) =>
                hotel.regionType &&
                hotel.regionType.toLowerCase().trim() ===
                  "international exchange",
            );

            setHotels(internalExchangeHotels);
            setFilteredHotels(internalExchangeHotels);
          }
        }
      } catch (err) {
      ...
      <div className="min-h-screen bg-black text-white pt-28 px-6 md:px-12 pb-20 font-sans">
        <div className="max-w-[1400px] mx-auto">
          <h1 className="text-[40px] md:text-[44px] font-bold mb-12 tracking-tight">
            Best Internal Exchange Resorts
          </h1>
