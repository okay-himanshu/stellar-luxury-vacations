...
export default function HotelsDashboard() {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      // Need token if admin API requires it, assuming you handle interceptors.
      // If not, ensure token is passed.
      ...
      if (resHotels.data.success) {
        ...
        // 👉 PAGINATION TOTAL PAGES FIX
        const totalItems = resHotels.data.data.total || 0;
        setTotalPages(Math.ceil(totalItems / ITEMS_PER_PAGE) || 1);
      }

      if (resCountries.data.success) {
        setCountries(
          resCountries.data.data.items || resCountries.data.data || [],
        );
      }

      if (resCities.data.success) {
        setCities(resCities.data.data.items || resCities.data.data || []);
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // Refresh current page. If it's the last item on the page, go to prev page.
  ...
  const handleSubmit = async (e) => {
    ...
    // Admin token from localStorage if you aren't using interceptors
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-6">
      ...
      {/* TABLE */}
      <div className="bg-[#1c1c1c] border border-gray-800 rounded-xl overflow-x-auto relative">
          ...
          <tbody className="text-gray-300 divide-y divide-gray-800">
            ...
            {hotels?.map((hotel) => (
              <tr key={hotel._id} className="hover:bg-gray-800/30">
                ...
                <td className="p-4">
                  <p className="font-bold text-white text-lg truncate max-w-xs">
                    {hotel.name}
                  </p>
